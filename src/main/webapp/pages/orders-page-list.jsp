...
<link rel="stylesheet"
	...
	<script>
		...
		function showMsgs(pageNum, pageSize) {
			//发送ajax请求
			$.get("${pageContext.request.contextPath}/findAllOrders/"+pageNum+"/"+pageSize, function (data) {
				ul = $("#tbody");
				ul.empty();
				$("#tbody").empty();
				for (var i=0;i<data.list.length;i++){
					...
					var $td=$(
							...
							"<td style=\"display:none\" name=ids>"+order.id+"</td>\n"+
							"<td>"+uid+"</td>\n" +
							"<td>"+order.orderNum+"\n" +
							"</td>\n" +
		...
		$("#delete").click(function () {
			...
			deleteProuduct(orderNums)
		});
		function deleteProuduct(orderNums) {
			$.ajax({
				url : "/crm/deleteOrder",
				type:"post",
				traditional: true,
					...
					<div class="box-body">
						...
						<div class="table-box">
							...
							<table id="dataList"
								...
								<tbody id="tbody">
<%--									<c:forEach items="${product.ordersList}" var="order">--%>
